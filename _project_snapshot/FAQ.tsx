          ...
          {faqs.map((faq, i) => (
            <AnimateOnScroll key={i} delay={i * 0.05}>
              ...
              >
                ...
                >
                  <span className={`text-sm sm:text-base font-medium pr-4 transition-colors ${openIndex === i ? "text-white" : "text-gray-300"}`}>
                    {faq.question}
                  </span>
                  <ChevronDown className={`w-5 h-5 shrink-0 text-gray-500 transition-transform duration-300 ${openIndex === i ? "rotate-180 text-indigo-400" : ""}`} />
                </button>
                <AnimatePresence>
                  {openIndex === i && (
                    <motion.div
                    ...
                    >
                      <p className="px-6 pb-5 text-sm text-gray-400 leading-relaxed">{faq.answer}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
